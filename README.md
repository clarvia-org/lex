...
# One law ID within a dataset root
uv run lex check --id lu/loi-2000-06-29-n2
```
...
| Country | Laws | Notes |
|---|---|---|
| Luxembourg (`lu`) | Stage 1B + Stage 2 batches 04–15 (1,207 IDs) | Partial — Casemates only; see `countries/lu/README.md` |
...
| **Projection fidelity** | `lex check` enforces statutory **word-token parity**: Markdown must not omit more than 0.5% of retained-source body words (catches silent legal prose loss). |
